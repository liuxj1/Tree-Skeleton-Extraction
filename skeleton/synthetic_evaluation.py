...
def process_skeletons(CMSBs, BPs, skeleton, name, all_accuracy,all_misdetection_rate,average_break, out_file, type):
...
if __name__ == "__main__":
    ...
    if Processing_mode == 'batch':
        CMSBs_path = "path/to/filefolder"  # Store the completed skeleton
        BPs_path = "path/to/filefolder"  # Folder for storing branch point pairs used for reconnection
        skeleton_path = "path/to/filefolder"  # The complete skeleton folder for the simulated point cloud
        output_file_path = "path/to/result.txt" # Used to store synthetic data set evaluation results
        all_accuracy = []
        all_misdetection_rate = []
        average_break = []

        with open(output_file_path, "w") as output_file:
            for filename in tqdm(os.listdir(CMSBs_path), desc="Processing labels"):
                if filename.endswith('.json'):
                    CMSBs = os.path.join(CMSBs_path, filename)

                    base_filename = os.path.basename(CMSBs)
                    name = base_filename.split('_CMSBs')[0]
                    BPs = f"{BPs_path}/{name}_BPs.txt"
                    skeleton = f"{skeleton_path}/{name}_imitaet_S.json"
                    process_skeletons(CMSBs, BPs, skeleton, name, all_accuracy, all_misdetection_rate,average_break, output_file, type)
        ...
        CMSBs = "../datas/CMSBs/simple_example3_CMSBs.json"  # Store the completed skeleton
        BPs_path = "../datas/BPs/simple_example3_BPs.txt"  # Folder for storing branch point pairs used for reconnection
        skeleton_path = "../datas/skeleton/simple_example3_imitaet_S.json"  # The complete skeleton folder for the simulated point cloud
        output_file_path = "../datas/simple_example2_result.txt"  # Used to store synthetic data set evaluation results
        all_accuracy = []
        all_misdetection_rate = []
        average_break = []
        ...
        with open(output_file_path, "w") as output_file:
            base_filename = os.path.basename(CMSBs)
            name = base_filename.split('_CMSBs')[0]
            process_skeletons(CMSBs, BPs_path, skeleton_path, name, all_accuracy, all_misdetection_rate, average_break,
                              output_file, type)
            average = sum(average_break) / len(average_break)
            average_accuracy = sum(all_accuracy) / len(all_accuracy)
            average_misdetection_rate = sum(all_misdetection_rate) / len(all_misdetection_rate)
