...
class LaplacianBasedContractionBase(SkeletonBase):
    ...
    def visualize(self):
        ...
        def create_spheres_at_nodes(graph, radius=0.002):
            spheres = []
            for node in graph.nodes():
                sphere = o3d.geometry.TriangleMesh.create_sphere(radius=radius)
                sphere.paint_uniform_color([0, 1, 0])
                sphere.translate(graph.nodes[node]['pos'])
                spheres.append(sphere)
            return spheres

        def visualize_topology():
            spheres = create_spheres_at_nodes(self.topology_graph)

            # Create a LineSet object to represent the edges of the graph
            line_set = o3d.geometry.LineSet()

            # Set the points of the line set
            points = np.asarray(self.topology.points)
            line_set.points = o3d.utility.Vector3dVector(points)

            # Set the lines (edges) of a line set using the edges of a minimum spanning tree
            lines = list(self.topology_graph.edges())
            ...
            custom_draw_geometry_with_no_light([line_set,pcd] + spheres)
...
if __name__ == "__main__":
    ...
    lbc = LBC(point_cloud=pcd, down_sample=0.0025)
